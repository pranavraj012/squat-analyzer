...
class ProcessFrame:
    ...
    def detect_camera_alignment(self, left_shldr_coord, right_shldr_coord, nose_coord):
        """
        STEP 1: CAMERA ALIGNMENT DETECTION
        Logic: Calculate the angle between shoulders and nose to determine if user is facing
        the camera (front view) or positioned sideways (side view).
        - If offset_angle > OFFSET_THRESH (35°): User is facing camera (front view)
        - If offset_angle ≤ OFFSET_THRESH: User is in proper side view position
        Returns: offset_angle (float)
        """
        offset_angle = find_angle(left_shldr_coord, right_shldr_coord, nose_coord)
        return offset_angle

    def select_optimal_side_profile(self, left_coords, right_coords):
        """
        STEP 2: SIDE PROFILE SELECTION
        Logic: Automatically select the side with better visibility by comparing
        the vertical distance between shoulder and foot on each side.
        The side with greater distance is more visible to the camera.
        """
        left_shldr_coord, left_elbow_coord, left_wrist_coord, left_hip_coord, \
        left_knee_coord, left_ankle_coord, left_foot_coord = left_coords
        right_shldr_coord, right_elbow_coord, right_wrist_coord, right_hip_coord, \
        right_knee_coord, right_ankle_coord, right_foot_coord = right_coords

        # Calculate visibility distances
        ...
        if dist_l_sh_hip > dist_r_sh_hip:
            # Use left side profile
            return (left_shldr_coord, left_elbow_coord, left_wrist_coord, left_hip_coord, 
                   left_knee_coord, left_ankle_coord, left_foot_coord), -1
        else:
            # Use right side profile
            return (right_shldr_coord, right_elbow_coord, right_wrist_coord,
                   right_hip_coord, right_knee_coord, right_ankle_coord, right_foot_coord), 1

    def calculate_vertical_angles(self, shldr_coord, hip_coord, knee_coord, ankle_coord):
        """
        STEP 3: VERTICAL ANGLE CALCULATIONS
        Logic: Calculate angles between body segments and vertical reference lines.
        These angles are crucial for analyzing squat form and depth.
        """
        # Hip angle: shoulder -> vertical line -> hip
        hip_vertical_angle = find_angle(shldr_coord, np.array([hip_coord[0], 0]), hip_coord)
        # Knee angle: hip -> vertical line -> knee  
        knee_vertical_angle = find_angle(hip_coord, np.array([knee_coord[0], 0]), knee_coord)
        # Ankle angle: knee -> vertical line -> ankle
        ankle_vertical_angle = find_angle(knee_coord, np.array([ankle_coord[0], 0]), ankle_coord)
        return hip_vertical_angle, knee_vertical_angle, ankle_vertical_angle

    def _get_state(self, knee_angle):
        """
        STEP 4: SQUAT STATE DETECTION
        Logic: Classify current squat position based on knee vertical angle.
        States:
        - s1 (NORMAL): 0-32° - Standing/starting position
        - s2 (TRANS): 35-65° - Transition/mid-squat
        - s3 (PASS): 70-95° - Deep squat position
        """
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                # Use the enhanced side profile selection logic
                left_coords = (left_shldr_coord, left_elbow_coord, left_wrist_coord, left_hip_coord, 
                              left_knee_coord, left_ankle_coord, left_foot_coord)
                right_coords = (right_shldr_coord, right_elbow_coord, right_wrist_coord, right_hip_coord, 
                               right_knee_coord, right_ankle_coord, right_foot_coord)
                
                # STEP 2: Select optimal side profile using enhanced logic
                selected_coords, multiplier = self.select_optimal_side_profile(left_coords, right_coords)
                shldr_coord, elbow_coord, wrist_coord, hip_coord, knee_coord, ankle_coord, foot_coord = selected_coords

                # STEP 3: Calculate vertical angles using enhanced logic
                hip_vertical_angle, knee_vertical_angle, ankle_vertical_angle = self.calculate_vertical_angles(
                    shldr_coord, hip_coord, knee_coord, ankle_coord)

                # Draw visual feedback - vertical reference lines
                ...
                if current_state == 's1':
                    # Enhanced squat counting logic for better accuracy
                    
                    # Perfect squat: complete sequence s1->s2->s3->s1 with good form
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        # Verify the sequence is correct: should contain s2 and s3
                    ...
                    # Partial squat: only reached s2, never got to s3 (shallow squat)
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq']) == 1:
                        self.state_tracker['IMPROPER_SQUAT'] += 1
                        play_sound = 'incorrect'

                    # Form errors detected during squat
